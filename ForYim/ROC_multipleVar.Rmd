...
########################################## function start #############################
raw.data <- read_delim(file.in,delim = "\t")

file.name <- basename(file.in)
file.name <- unlist(strsplit(file.name,"\\.txt"))
dir.name <- dirname(file.in)
dir.out <- paste0(dir.name,"/output_",file.name)
...
screen_division <- floor(sqrt(var_col_end-var_col_start))
target_origianl <- colnames(ROC_data)[target_col]
colnames(ROC_data)[target_col] <- "event"


cairo_pdf(file.path(dir.out,"ROCplot.pdf"),width = 20, height = 20,pointsize = 20)

par(mfrow=c(screen_division+1,screen_division))
for(i in c(var_col_start:var_col_end)){

var_originalanme <- colnames(ROC_data)[i]
colnames(ROC_data)[i] <- "var_independent"
main.title <- paste0(target_origianl," ~ ",var_originalanme)
a1 <- ROC(form = event~var_independent, data = ROC_data,plot = F)


plot_ROC(a1,show.lr.eta = F,main.title = main.title)
colnames(ROC_data)[i] <- var_originalanme
}
par(mfrow=c(1,1))
