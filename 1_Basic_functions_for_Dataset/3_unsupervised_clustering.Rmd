...
# Copy the codes and paste and Run~
```{r}
...
#------------- Packages ----
packages <- c("dplyr", "reshape2","data.table","dendextend","amap","gplots","magick","readr","shiny")
instPak (packages) 
#-----------------------------
...
server <- function(input, output, session) {
  ...
  reactiveHorizontalCluster.hc_dataClu <- reactive({
    d_dataClu <- Dist(as.matrix(reactiveMatrixTranposition()[-1]), method = distanceOption)
    hc_dataClu <- hclust(d_dataClu, method = linkOption)
  })
  reactiveHorizontalCluster.hc_dataClu_r <- reactive({
    d_dataClu_r <- Dist(as.matrix(reactivefiltering()[-1]), method = distanceOption)
    hc_dataClu_r <- hclust(d_dataClu_r, method = linkOption)
  })
  reactiveMatrixTranposition <- reactive({
    matrixTranspositionXY(reactivefiltering())
  })
  ...
  doClustering <- eventReactive(input$doClustering,{
    ...
    cluster.data <- reactiveMatrixTranposition()
    ...
    file.cutting.gene <<-reorder.dend(file.cutting.gene)[-2]
    colnames(file.cutting.gene) <<- c("gene","group")
    
    # cdt file creation
    dirOut <- "cdts"
    if(!file.exists(dirOut))dir.create(dirOut)
    r2atr(reactiveHorizontalCluster.hc_dataClu(), file=file.path(dirOut,"cluster.atr"))
    r2gtr(reactiveHorizontalCluster.hc_dataClu_r(), file=file.path(dirOut,"cluster.gtr"))
    r2cdt(reactiveHorizontalCluster.hc_dataClu_r(),reactiveHorizontalCluster.hc_dataClu(),as.matrix(reactivefiltering()[-1]), file=file.path(dirOut,"cluster.cdt"))
    tarfile <- 'newfile.tgz'
  ...
  output$downloadclustergene <- downloadHandler(
    filename = function() {
      "ClusteredGene.txt"
    },
    content = function(file) {
      
      contents.table <- file.cutting.gene
      write_delim(contents.table, file, delim = "\t",na = "")
    },
    contentType = "text/plain"
  )
  ...
  output$downloadcdt <- downloadHandler(
    filename = function(){'cdtFiles.tgz'},
    content = function(file) {
...
ui <- fluidPage(titlePanel("UnSupervised Clustering"),
                sidebarLayout(
                  sidebarPanel(
                    ...
                    downloadButton('downloadclusterGroup', 'Sample'),
                    downloadButton('downloadclustergene', 'Gene'),
                    br(),
                    br(),
                    downloadButton('downloadDataHeatmap', 'Download Heatmap'),
                    downloadButton('downloadcdt', 'Download cdt files'),
                    br(),
                    br()
                  ),
                  mainPanel(
