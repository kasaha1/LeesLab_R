...
server <- function(input, output, session) {
  ...
  doClustering <- eventReactive(input$doClustering,{
    ...
    files2zip <- dir('cdts', full.names = TRUE)
    zip(zipfile = 'cdts', files = files2zip)
  ...
  output$downloadcdt <- downloadHandler(
    filename = function(){'cdtFiles.zip'},
    content = function(fname) {
      
      fs <- c("cluster.atr", "cluster.gtr", "cluster.cdt")
      r2atr(reactiveHorizontalCluster.hc_dataClu(), file="cluster.atr")
      r2gtr(reactiveHorizontalCluster.hc_dataClu_r(), file = "cluster.gtr")
      r2cdt(reactiveHorizontalCluster.hc_dataClu_r(),reactiveHorizontalCluster.hc_dataClu(),as.matrix(reactivefiltering()[-1]), file = "cluster.cdt")
      print (fs)
