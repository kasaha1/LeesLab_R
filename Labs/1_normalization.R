...
#------------- Packages ----
packages <- c("ggplot2", "dplyr", "reshape2","moonBook", "readr","colorspace","shiny")
instPak (packages) 
#-----------------------------
...
ui <- fluidPage(titlePanel("Uploading Files"),
                sidebarLayout(
                  sidebarPanel(
                    ...
                    checkboxInput("Flooring_after_Log2", label = "Flooring under 1", value = TRUE),
                    checkboxInput("doLog2", label = "Log2", value = TRUE),
                    actionButton("action1", "Do", class = "btn-primary"),
                    downloadButton('downloadData', 'Download')
                  ),
                  mainPanel(plotOutput('plotContents1'),br(),plotOutput('plotContents2'))
                ))
